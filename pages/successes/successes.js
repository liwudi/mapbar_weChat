...
Page({
  ...
  onLoad:function(options){
    ...
    AppService.getUserInfo().then(res => {
      ...
      _this.setData({
        ...
        one_max_distance:res.data.user.maxDistance==null?0:(res.data.user.maxDistance).toFixed(3),
        all_city:res.data.user.totalCitys==null?0:res.data.user.totalCitys,
        history_distance:res.data.user.totalDistance==null?0:res.data.user.totalDistance.toFixed(3),
      });
    ...
    AppService.getUsersList(_this.data.groupId,1).then(res => {
      ...
      let mylist = list.map((item, i) => {
        return {
          ...
          time:((item.updateTime-item.creatTime)/60000).toFixed(1),
          distance:parseInt(item.distance/1000),
        }
      });
      _this.setData({
        usersList:mylist
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      path: `/pages/successes/successes?isShare=true&groupId=${_this.data.groupId}&userId=${_this.data.userId}&destination=${_this.data.destination}&lat=${_this.data.my_lat}&lon=${_this.data.my_lon}&deslon=${_this.data.destlon}&deslat=${_this.data.destlat}`
    }
  },
})